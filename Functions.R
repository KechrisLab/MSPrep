...
readdata<-function(directory, clinicalfile, quantificationfile, linkfile, cvmax=0.50, missing=1, linktxt){
...
temp5<-matrix(NA, ncol=2, nrow=subjects*compounds)
k<-1
...
for (i in 1:subjects){
	...
	for (m in 1:replicates) {
		bcall<-paste(bcall, "metab[,i*replicates-",eval(m-1),"],",sep="")
	}
	bcall2<-paste("matrix(c(",substr(bcall,1,nchar(bcall)-1),"),ncol=",replicates,")",sep="")
	...
	for (j in 1:compounds){
		if (replicates==1) {
			...
			for (m in 1:replicates) {
				if (temp[j,m]==missing){temp[j,m]<-NA}
				}
			temp2[j,1]<-mean(temp[j,],na.rm=TRUE)
			temp2[j,2]<-sd(temp[j,],na.rm=TRUE)
			temp2[j,3]<-temp2[j,2]/temp2[j,1]
			if (is.na(temp2[j,3])) { temp2[j,3] <- cvmax + 1 }
			temp2[j,4]<-length(na.omit(temp[j,]))
			if (temp2[j,1]=="NaN"){temp2[j,1]<-0}
			### Compound must be present in at least half replicates
			### Compound must have CV less than CVMax
			if (temp2[j,4] < ceiling(replicates/2) || temp2[j,3] > cvmax){
				temp2[j,1]<-0
				if (temp2[j,4] == replicates) {
		...
		### temp2[,3] = CV
		### temp2[,4] = n
		temp5[eval((i*compounds)-(compounds-1)):eval(i*compounds),1:2]<-temp2[,3:4]
		}
	if (replicates!=1) {
	...
	list(sum_data1=metaf, clinical=clinical, medians=med_comp, cv_summary=temp5)
}
...
filterft<-function(metaf,filterpercent=0.50, npcs=3) {
	...
	metabpca <- pca(t(metaimp), nPcs=npcs, method="bpca")

	bpca<-t(completeObs(metabpca))
...
graphimputations<-function(directory, minval, withzero, bpca, meanval1=0, meanval2=1000000, xmax1=500000, ymax1=25000, xmax2=20, ymax2=1000, xmax3=20, ymax3=1500, nbreaks=500) {
...
combat<-function(dset, pheno, batch) {
	...
	comadj<-t(ComBat(d1mod, mod=d2a, batch=d3))
...
diagnosticgen<-function(testobj, clindat, link1, batch, pheno, directory, ylim1, ylim2, ylim3) {
...
temp2<-data_crmn
boxplot(as.numeric(temp2[,1])~temp2[,2],  main="", xlab=xlab1, ylab="Abundance",ylim=ylim3)
boxplot(as.numeric(temp2[,1])~temp2[,3],  main="", xlab=xlab2, ylab="Abundance", ylim=ylim3)
boxplot(as.numeric(temp2[,1])~temp2[,4],  main="", xlab=xlab3, ylab="Abundance", ylim=ylim3)

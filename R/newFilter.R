msFilter <- function(data,
    ...
    if (is(data, "SummarizedExperiment")) {
        ...
        rtn <- .dfFilter(data, filterPercent, compVars, techVars, colExtraText,
...
.seFilter <- function(SE, filterPercent, missingValue, returnToDF) {
    ...
    ## Store existing metadata, get compVars and techVars 
    ...
    ## Return Data
    rtn <- .msReturn(filteredData, compVars, techVars, metaData, 
                     toSE = !returnToDF)
}

.dfFilter <- function(data, filterPercent, compVars, techVars, colExtraText,
                      separator, missingValue, returnToSE) {
    ## Tidy Data
    tidyData <- msTidy(data = data, compVars = compVars, techVars = techVars,
                       colExtraText = colExtraText, separator = separator,
    ...
    ## Return Data
    rtn <- .msReturn(filteredData, compVars, techVars, toSE = returnToSE)
}
